...
class OpenPayoutRequestDto(BaseModel):
    """
    ...
    wallet_id: Optional[StrictStr] = Field(default=None, description="如果不传，就使用Api-Token中商户的交易钱包。如果是共享钱包，则必须要传", alias="walletId")
    merchant_id: Optional[StrictStr] = Field(default=None, description="如果不传，就用Api-Token中的商户", alias="merchantId")
    __properties: ClassVar[List[str]] = ["verification", "requestTime", "version", "nonce", "orderId", "title", "description", "payeeName", "payeeBankCode", "payeeBankAccNo", "amount", "notifyUrl", "remark", "currency", "walletId", "merchantId"]
