...
class CreateMerchantSelfRechargeDto(BaseModel):
    """
    CreateMerchantSelfRechargeDto
    """ # noqa: E501
    source_currency: StrictStr = Field(description="货币类型", alias="sourceCurrency")
    source_amount: Annotated[str, Field(strict=True)] = Field(alias="sourceAmount")
    target_currency: StrictStr = Field(description="货币类型", alias="targetCurrency")
    ...
    user_submited: Optional[StrictBool] = Field(default=None, alias="userSubmited")
    received: Optional[StrictBool] = None
    __properties: ClassVar[List[str]] = ["sourceCurrency", "sourceAmount", "targetCurrency", "exchangeRateId", "exchangeRate", "exchangeRateSource", "payeeWalletAddress", "targetAmount", "amount", "merchantId", "note", "timeout", "userSubmited", "received"]

    @field_validator('source_currency')
    def source_currency_validate_enum(cls, value):
        """Validates the enum"""
    ...
    def target_currency_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['NGN', 'GHS', 'ETH', 'BTC', 'USDT']):
            raise ValueError("must be one of enum values ('NGN', 'GHS', 'ETH', 'BTC', 'USDT')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "userSubmited": obj.get("userSubmited"),
